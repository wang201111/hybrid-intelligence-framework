...
class AugmentationConfig:
    ...
    # Output settings
    output_dir: Path = PROJECT_ROOT / 'results' / 'solubility' / 'ablation' / 'IADAF_results'
...
class SingleFoldRunner:
    ...
    def _train_dnn_tstr(
        ...
        Uses TSTREvaluator exactly as in original IADAF ablation experiment.
        
        Args:
            X_synthetic: Synthetic data [n_synthetic, 3] - [T, W_comp1, W_comp2].
            X_val: Real validation features [n_val, 2].
            y_val: Real validation targets [n_val].
            X_test: Real test features [n_test, 2].
            y_test: Real test targets [n_test].
        ...
        Returns:
            ...
            - history: Training history dict with train/val/test R² curves
        """
        # Split synthetic data into features and target
        X_syn_train = X_synthetic[:, :2]  # [T, W_comp1]
        y_syn_train = X_synthetic[:, 2]   # W_comp2
        
        # Create TSTREvaluator with config (exactly as in original code)
        evaluator = TSTREvaluator(
            ...
            config=PhysicsConfig(
                ...
                dnn_layer_dim=4,
                dnn_node_dim=128,
                tstr_device=self.device,
                best_model_start_epoch=500
            )
            # Note: Do NOT pass device parameter directly
            # (standardized version uses config.tstr_device)
        )
        
        # Train model using TSTR paradigm
        ...
        # Build history dict (keep consistent format with original)
        history = {
            "train_r2": result['history']['train_r2'],
            "val_r2": result['history']['val_r2'],
            "test_r2": result['history']['test_r2']
        }
        
        # Log completion
        best_val_r2 = result['metrics']['val_r2']
        self.logger.info(f"TSTREvaluator training complete (Val R² = {best_val_r2:.6f})")
    ...
    def _evaluate_dnn(
        ...
        def predict(X):
            ...
            with torch.no_grad():
                pred = model(torch.FloatTensor(X_scaled).to(device)).cpu().numpy()
            return y_scaler.inverse_transform(pred).flatten()
    
        preds = {s: predict(X) for s, X in
                 [('train', X_train), ('val', X_val), ('test', X_test)]}
        trues = {'train': y_train, 'val': y_val, 'test': y_test}
        ...
        self.logger.info(
            f"Train R²={metrics_result['train_r2']:.4f}, "
            f"Val R²={metrics_result['val_r2']:.4f}, "
            f"Test R²={metrics_result['test_r2']:.4f}"
        )
